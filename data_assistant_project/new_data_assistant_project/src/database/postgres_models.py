        ...
        try:
            ...
            cursor.execute(
                ...
                INSERT INTO comprehensive_feedback (
                    ...
                    learning_improvement_rating, learning_improvement_reason, manual_trigger,
                    manual_reason, auto_explanation, auto_reason, system_accuracy, 
                    system_accuracy_index, recommendation, recommendation_index, created_at
        ...
        try:
            ...
            cursor.execute(
                ...
                SELECT cf.id, cf.user_id, cf.frequency_rating, cf.frequency_reason,
                       ...
                       cf.manual_trigger, cf.manual_reason, cf.auto_explanation, cf.auto_reason, 
                       cf.system_accuracy, cf.system_accuracy_index, cf.recommendation, 
                       cf.recommendation_index, cf.created_at, u.username
            ...
            for row in rows:
                fb = cls(
                    ...
                    manual_trigger=bool(row[10]) if row[10] is not None else None,
                    manual_reason=row[11],
                    auto_explanation=bool(row[12]),
                    auto_reason=row[13],
                    system_accuracy=row[14],
                    system_accuracy_index=row[15],
                    recommendation=row[16],
                    recommendation_index=row[17],
                    created_at=row[18]
                    if isinstance(row[18], datetime)
                    else datetime.fromisoformat(str(row[18])),
                )
                setattr(fb, "username", row[19])
                feedback_list.append(fb)
            return feedback_list
        except Exception as e:
